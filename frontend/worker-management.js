...
async function loadWorkersFromDb(){
    try{
        ...
        if(Array.isArray(list)){
            ...
            // sync config.assignees for other modules
            if(window.config && Array.isArray(window.config.assignees)){
                window.config.assignees.length = 0;
                workers.forEach(w=>window.config.assignees.push({id:w.id,name:w.name}));
            }
        }
...
// Ensure that each worker document exists in the backend DB (Mongo)
async function ensureWorkersSyncedToDb() {
    if (!Array.isArray(workers) || workers.length === 0) return;
    // Fire-and-forget: we don't await every call, but ensure at least one async chain
    try {
        await Promise.all(workers.map(w => updateWorkerInDb(w)));
    } catch (e) {
        console.warn('ensureWorkersSyncedToDb: syncing workers failed', e);
    }
}

// Initialize workers from config.assignees if not already initialized
async function initializeWorkers() {
    if (workers.length === 0 && window.config && window.config.assignees) {
        // Try to load workers from the database first
        try {
            await loadWorkersFromDb();
            if (workers.length === 0) {
                // If no workers in DB, create default ones from config
                workers = window.config.assignees.map(assignee => ({
                    id: assignee.id,
                    name: assignee.name,
                    monthlyPay: 1600, // Default monthly pay
                    presence: {},
                    note: ''
                }));
                // Save new workers to MongoDB
                await Promise.all(workers.map(worker => updateWorkerInDb(worker)));
            }
        } catch (error) {
...
function resetSummaryDateRange() {
    ...
    // Update the date range label instead of trying to set input values
...
function initializeDateRangeInputs() {
    // No more direct input elements; update the label instead
    updateDateRangeLabel();
}
...
function renderMiniCalendar(year, month) {
    ...
    const baseDay = new Date(year, month, 1);
    ...
    prevBtn.textContent = '<';
    prevBtn.onclick = () => renderMiniCalendar(year, month - 1);
    const nextBtn = document.createElement('button');
    nextBtn.textContent = '>';
    nextBtn.onclick = () => renderMiniCalendar(year, month + 1);
    ...
    for (let day = 1; day <= lastDay.getDate(); day++) {
        ...
        if (calendarStart && calendarEnd && d >= calendarStart && d <= calendarEnd) {
            cell.style.background = '#007bff';
            cell.style.color = '#fff';
            cell.style.borderRadius = '4px';
        } else if (calendarStart && d.getTime() === calendarStart.getTime()) {
            cell.style.background = '#007bff';
            cell.style.color = '#fff';
            cell.style.borderRadius = '50%';
        } else if (calendarEnd && d.getTime() === calendarEnd.getTime()) {
            cell.style.background = '#007bff';
            cell.style.color = '#fff';
            cell.style.borderRadius = '50%';
        }
        cell.onclick = () => {
            if (!calendarSelecting) {
                ...
                // Set summaryDateRange and update
                summaryDateRange.start = setToMidnight(calendarStart);
                summaryDateRange.end = setToMidnight(calendarEnd);
                updateDateRangeLabel();
                updateMonthlySummary();
                document.getElementById('mini-calendar').style.display = 'none';
                calendarSelecting = false;
            }
...
// Show/hide mini calendar on box click
const dateRangeBox = document.getElementById('date-range-box');
if (dateRangeBox) {
    dateRangeBox.addEventListener('click', () => {
        const miniCal = document.getElementById('mini-calendar');
        if (miniCal.style.display === 'none') {
            // Set initial selection to current summaryDateRange
            calendarStart = summaryDateRange.start;
            calendarEnd = summaryDateRange.end;
            calendarSelecting = false;
            renderMiniCalendar(calendarStart.getFullYear(), calendarStart.getMonth());
...
function calculateWorkerStats(worker) {
    ...
    workingDaysInRange.forEach(d => {
        ...
        if (presence && presence.present && presence.hours !== -baseHours) {
            ...
            // present but full negative hours counts as absence but still supHours effect
            supHours += presence.hours;
        }
    });
...
// Full salary if all scheduled working days are worked
// Absences and extra days/hours handled as supplementary
function calculateMonthlyPay(worker) {
    ...
    workingDaysInRange.forEach(date => {
        ...
        if (presence && presence.present) {
            workedDays++;
            // Any deviation from base hours is supplementary
    ...
    // New salary rules:
    // dailyPay = monthlyPay / 26
    // hourlyRate = dailyPay / baseHours
    // total = dailyPay * workedDays ± supHours * hourlyRate
...
function updateCalendar() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        workers.forEach(worker => {
            ...
            const baseHours = calculateWorkingHours(workingHours.start, workingHours.end);
            if (!presence) {
                // Start with 0 hours when absent to avoid unintended supplementary hours
            ...
            if (groupEditMode || presence.present) {
                ...
                if (groupEditMode) {
                    ...
                    // Presence slider identical to day edit modal
                    const switchDiv = document.createElement('div');
                    switchDiv.className = `presence-switch ${presence.present ? 'active' : ''}`;
                    ...
                    switchDiv.onclick = () => {
                        presence.present = !presence.present;
                        switchDiv.classList.toggle('active');
                        // Reset supplementary hours to 0 when toggling
                        presence.hours = 0;
                    ...
                    // Add sup hours display when not in group mode
...
function updateMonthlySummary() {
    ...
    workers.forEach(worker => {
        ...
        // Change from right-click to hover for calculation popup
        let popupTimeout;
        summary.addEventListener('mouseenter', (e) => {
            popupTimeout = setTimeout(() => showCalculationPopup(worker, e), 500); // 500ms delay
        });
...
// Clean up data older than MAX_HISTORY_MONTHS
function cleanupOldData() {
    const cutoffDate = new Date();
    cutoffDate.setMonth(cutoffDate.getMonth() - MAX_HISTORY_MONTHS);
    
    workers.forEach(worker => {
        for (const dateStr in worker.presence) {
...
function openEmployeeModal() {
    ...
    // In openEmployeeModal, only show edit button for user-added employees
    workers.forEach(worker => {
        if (!worker) return; // Skip invalid workers
...
function closeEmployeeModal() {
    ...
    if (nameInput) nameInput.disabled = false; // re-enable for next add
    document.getElementById('form-title').textContent = 'Add New Employee';
}

// Toggle modifier mode for a worker
...
function toggleModifier(workerId, btn){
    ...
    // Enter modifier mode for this worker
...
// (deprecated) editEmployee kept for compatibility but now calls toggleModifier
function editEmployee(workerId){
    const dummy=document.createElement('button');
    toggleModifier(workerId,dummy);
}
...
async function deleteEmployee(workerId) {
    if (confirm('Are you sure you want to delete this employee?')) {
        ...
        // Remove from config.assignees as well
        if (worker && window.config && Array.isArray(window.config.assignees)) {
            const idx = window.config.assignees.findIndex(a => a.id === workerId || a.name === worker.name);
            if (idx !== -1) {
                window.config.assignees.splice(idx, 1);
                if (typeof saveSettings === 'function') {
                    try { saveSettings(); } catch (e) { console.warn('saveSettings failed', e); }
                }
            }
        }
        saveWorkers();
        updateCalendar();
        updateMonthlySummary();
        openEmployeeModal(); // Refresh the list
    }
}
...
document.getElementById('employee-form').addEventListener('submit', async function (e) {
    ...
    if (currentModifierId) {
        // We are editing an existing worker.
        ...
        if (createdWorker && createdWorker._id) {
            ...
            // Handle error, e.g., worker with that name already exists.
        }
    }
    ...
    updateCalendar();
    updateMonthlySummary();
    // If a day-edit modal is open, refresh it to reflect name changes
    if (selectedDate) {
        const tempDate = selectedDate; // preserve
        closeDayEditModal();
        openDayEditModal(tempDate);
    }
    openEmployeeModal(); // Refresh the employee list
});
...
function showCalculationPopup(worker, event) {
    ...
    popup.style.pointerEvents = 'none'; // Prevent popup from interfering with hover
    popup.innerHTML = `
...
function initializePage() {
    ...
    initializeWorkers();
    // Sync any locally stored workers to the database on page load
    ensureWorkersSyncedToDb();
